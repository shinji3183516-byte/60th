...
function runAllCarsParade() {
  ...
  departing.forEach((entry, index) => {
    // 周回リセット時だけ退出速度を落とす（通常のpush退出速度は変更しない）。
    window.setTimeout(() => removeActiveRunCar(entry, true, 2000), index * 90);
  });
  ...
  }, Math.min(LOOP_RESET_HOLD_MS - 200, 3400));
}
    ...
    grouped.forEach((entries, id) => {
      ...
      entries.forEach(({ row }) => {
        ...
        if (type === "IMAGE") {
          blocks.push({
            type: "image",
            src: resolveTimelineImage(value, imageFolder),
